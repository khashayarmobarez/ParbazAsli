    ...
    return (
      <ThemeProvider theme={theme}>
        ...
        >
          ...
          >
            <BottomNavigationAction onClick={() => navigate('/profile')} label={<Typography variant="body1" sx={{ fontSize: '12px', marginTop:'2px' }}>پروفایل</Typography>} icon={<PersonOutlineOutlinedIcon sx={{width:'1.28rem', height:'1.28rem'}} />} sx={{width:'100%' ,'&.Mui-selected': { color: currentUrl.includes('/profile') ? 'var(--text-accent)' : 'inherit' }, color:'var(--neutral-light)'}} />
            ...
            <BottomNavigationAction onClick={() => navigate('/flightHistory')} label={<Typography variant="body1" sx={{ fontSize: '12px', marginTop:'2px' }}>لاگ بوک</Typography>} icon={<FolderOutlinedIcon sx={{width:'1.28rem', height:'1.28rem'}}/>} sx={{width:'100%' ,'&.Mui-selected': { color: currentUrl.includes('/flightHistory') ? 'var(--text-accent)' : 'inherit' }, color:'var(--neutral-light)'}} />
          </BottomNavigation>
        </Box>
      </ThemeProvider>
    );
