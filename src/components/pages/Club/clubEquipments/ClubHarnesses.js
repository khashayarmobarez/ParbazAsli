...
const ClubHarnesses = (props) => {

    const navigate = useNavigate()

    const { data: userEquipmentsData, isLoading, error } = useUserEquipments(3, true)
    ...
    return (
        <div className=' flex flex-col gap-y-12 items-center'>
                ...
                userEquipmentsData.data.map(equipment =>
                        <div key={equipment.id} className={`w-full justify-between items-center px-5 py-4 rounded-[1.6rem] flex flex-col gap-y-6 md:col-span-1`} style={{background:'var(--organs-coachData-bg', boxShadow:'var(--organs-coachData-boxShadow)'}}>
                            ...
                            <div className=' w-full text-xs flex justify-between items-start gap-y-1'>
                                <p> برند {equipment.brand} / مدل {equipment.model} / سایز{equipment.size}</p>
            ...
            <Link to='/club/clubEquipment/harnesses' onClick={handleDisabledButton} className='fixed bottom-[3.2rem] w-[90%] bg-[#131423] rounded-xl md:w-96 md:relative md:bottom-0 md:top-4 h-[56px] '>
